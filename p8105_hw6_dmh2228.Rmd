...
One using mother's age at delivery (in years) and average number of cigarettes smoked per day during pregnancy as predictors (main effects only). This model is based on factors that underly birthweight.

```{r}
birthweight |>
  lm(bwt ~ momage + smoken, data = _) |>
  broom::tidy()
```
...
```{r}
birthweight |>
  lm(bwt ~ bhead + blength + babysex + bhead*blength*babysex, data = _) |>
  broom::tidy()
```
